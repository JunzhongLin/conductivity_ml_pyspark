...
def _train_test_split(spark, configs):
    ...
    df = spark.read.parquet('./data/transformed_data.parquet')
    ...
    train_df.drop(col('RO300_stratified_label')).write.mode('overwrite').parquet('./data/ml_dataset/train.parquet')
    test_df.drop(col('RO300_stratified_label')).write.mode('overwrite').parquet('./data/ml_dataset/test.parquet')
...
def _extract_data(spark):
    ...
    train_df = spark.read.parquet('./data/ml_dataset/train.parquet')
    test_df = spark.read.parquet('./data/ml_dataset/test.parquet')
...
def main():
    ...
    spark, log, configs = start_spark(
        app_name='conductivity_ml_job',
        files=['./app/configs/ml_config.json'])
